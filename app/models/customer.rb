class Customer < ApplicationRecord
  ...
  def self.create_samples
    ...
    customer_number.times do |i|
      ...
      if customer.save!
        meals.each do |meal|
          order_meal = customer.ordered_meals.new(meal_id: meal.id, ideal_served_time: next_serve_time, is_started: false, is_rescheduled: false)
          order_meal.save
          next_serve_time += ((meal.eating_time + meal.interval) * customer.style.velocity_params).round * 60
        end
      end
    end
  end
